...
max_iter = 500  # maximum number of optimization iterations
...
for i in range(max_iter):
	...
	if i > min_iter and (lik_list[-2] - lik_list[-1]) / lik_list[-2] < tol:
		break
...
print()
print('Optimization time for N=%d, K=%d, overlap=%.2f, mu_in=%.1f, E=%d, C=%d, gamma=%d, epsilon_max=%.1f: %ds for data parsing, %ds for tensor construction, %ds for inference, %ds for evaluation' % (N, K, overlap, mu_in, E, C, gamma, epsilon_max, parse_time, tensor_time, infer_time, eval_time))
...
res_dict['roc_auc'] = roc_auc_list
res_dict['prc_auc'] = prc_auc_list
