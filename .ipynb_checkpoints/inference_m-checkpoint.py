...
ratio = 16  # <RW> cascade-edge ratio
T = 10  # ending time

# Single layer result processing parameters
s_max_iter = 500  # <RW> maximum number of optimization iterations in the single layer phase
E_ratio = 1.1  # ratio of number of inferred edges over ground-truth edges

# Optimization parameters (multilayer phase)
s_c = 8  # <W> cascade size threshold for filtering out small cascades
seeds = [0, 1, 2]  # <W> seeds for optimization initialization in multiple runs
...
for seed in seeds:
	...
	print('\nAlpha correlation: %.8f' % corr_best)
	print('Alpha AUC (ROC): %.8f' % roc_auc_best)
	print('Alpha AUC (PRC): %.8f\n' % prc_auc_best)

	end_time = time.time()
	eval_time += end_time - start_time

	print('Optimization time for N=%d, K=%d, overlap=%.2f, mu_in=%.1f, E=%d, C=%d/%d, s_c=%d, gamma=%d, epsilon_max=%.1f: %ds for data parsing, %ds for tensor construction, %ds for inference, %ds for evaluation' % (N, K, overlap, mu_in, E, C, C_base, s_c, gamma, epsilon_max, parse_time, tensor_time, infer_time, eval_time))
